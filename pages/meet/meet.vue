<template>
	<view class="witness_content">
		<uni-swipe-action>
			<uni-swipe-action-item :options="options" @click="onClick($event,item)" @change="change" v-for="(item,i) in meetList"
			 :key="item.id" style="margin-top: 24upx;background-color: #F9F9F9;">
				<view class="resume_item" @click="navigateTo(`witness/witness?id=${item.id}`)">
					<view class="resume_content">
						<view class="gift_name">
							{{item.name}}
						</view>
						<view class="gift_img_box" v-if="item.resources.length">
							<image v-for="(list,index) in item.resources" class="gift_img_img" v-if="index<3" :key=list.id  :src="list.resFileName?imageBaseUrl+list.resFileName:'/static/index/Rectangle%2012%20Copy%202.jpg'"
		...
		<view class="cash_button">
			<button type="primary" @click="navigateTo(`createMeet/createMeet`)">创建主题</button>
		</view>
	</view>
</template>
